export const setPlaceholderText = () => {
    ...
    window.innerWidth < 400 ? (input.paceholder = "City, State, Country") : (input.placeholder = "City, State, Country, or Zip Code");
};
...
export const updateDisplay = (weatherJson, locationObj) => {
  ...
  const screenReaderWeather = buildSreenReaderWeather(
    weatherJson,
    locationObj
  );
...
const buildSreenReaderWeather = (weatherJson, locationObj) => {
...
const displaySixDayForecast = (weatherJson) => {
    for (let i = 1; i <= 6; i++) {
        const dfArray = createDailyForecatDivs(weatherJson.daily[i]);
        displayDailyForecast(dfArray);

    }
};

const createDailyForecatDivs = (dayWeather) => {
    ...
    const dayIcon = createDailyForecatIcon(dayWeather.weather[0].icon, dayWeather.weather[0].description);
...
const createDailyForecatIcon = (icon, altText) => {
